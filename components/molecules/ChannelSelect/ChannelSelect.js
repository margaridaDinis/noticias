...
import ChannelContext, { CHANGE_COUNTRY, CHANGE_CHANNEL_URL } from '../../../context/Channel';
...
const ChannelSelect = ({ closeModal }) => {
  ...
  const selectChannel = (newChannel, newCountry) => {
    if (country !== newCountry) dispatch({ type: CHANGE_COUNTRY, country: newCountry });
  ...
  return (
    <SectionList
      ...
      renderSectionHeader={({ section: { title, country } }) => (
        <View style={styles.section}>
          <Text style={styles.country}bold>{country}</Text>
          <Text style={styles.network} bold>{title}</Text>
        </View>
      )}
